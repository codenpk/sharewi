...
public class DepotList extends BasePage implements Serializable {
    private static final long serialVersionUID = 8189561997225150175L;
    ...
    private List<Depot> resultSet = null;                    //object
    ...
    public DepotList(LocationManager locationManager) {
        ...
        log.debug("Created a depotList bean");
    }
    ...
    public List<Depot> getDepots() {
        ...
        if (resultSet == null) {
            ...
            resultSet = locationManager.find(Depot.class, findProvinces, findCities, findPostalCodes);
        }
        return sort(resultSet);
    }

    public List<Depot> getResultSet() {
        return resultSet;
    }
    ...
    public Map<String, String> getProvinceSelectItems() {
        if (provinceSelectItems == null) {
            ...
            List<Depot> results = locationManager.find(Depot.class, null, null, null);
            Map<String, String> selectItems = new TreeMap<String, String>();
            for (Depot depot : results) {
                String province = depot.getAddress().getProvince();
    ...
    public Map<String, String> getCitySelectItems() {
        if (citySelectItems == null) {
            log.debug("Hitting DB");
            List<Depot> results = locationManager.find(Depot.class, findProvinces, null, null);
            Map<String, String> selectItems = new TreeMap<String, String>();
            for (Depot depot : results) {
                String city = depot.getAddress().getCity();
    ...
    public Map<String, String> getPostalCodeSelectItems() {
        if (postalCodeSelectItems == null) {
            log.debug("Hitting DB");
            List<Depot> results = locationManager.find(Depot.class, findProvinces, findCities, null);
            Map<String, String> selectItems = new TreeMap<String, String>();
            for (Depot depot : results) {
                String postalCode = depot.getAddress().getPostalCode();
    ...
    //Setters
    public void setResultSet(List<Depot> resultSet) {
